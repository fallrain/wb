  ...
  Wb = {
    ...
    getApp: function(request, response) {
      ...
      app = {
         ...
         * @param {Boolean} [isInvoke] 是否为invoke模式的调用。invoke模式运行后只返回闭包部分的脚本。
         * 如果指定该参数，将在独立虚拟的response内运行。
    ...
    dateToStr: function(date) {
      if (date instanceof JavaDate)
        ...
        return Wb.format('%d-%02d-%02d %02d:%02d:%02d.%d', date.getFullYear(), date.getMonth() + 1, date.getDate(),
     ...
     * 由于该方法运行不基于会话，因此被调用的模块会话相关的变量将不能使用。
     * @param {String} url 运行的模块url地址。
     * @param {Object} [params] 请求的参数对象。
     * @param {Boolean} [isInvoke] 是否为invoke模式的调用。invoke模式运行后只返回闭包部分的脚本。
     * @return {String} 返回的脚本。

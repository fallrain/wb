/*
Copyright (c) Geejing inc, all rights reserved.
...
var Wb = {
  ...
  toast: window.Ext && Ext.isTouch ? Ext.toast : function(message, topCenter) {
    ...
    if (Ext.isObject(message)) {
      html = message.html;
      topCenter = message.topCenter;
      config = Wb.exclude(message, 'html,topCener');
    } else {
      html = message;
      config = null;
    }
    new Ext.ux.window.Notification(Wb.apply({
      ...
      bodyStyle: 'line-height:20px;background:white;color:black;',
      autoCloseDelay: 4500,
      maxWidth: 500,
      border: false,
      frame: false,
  ...
  tip: function(message, isError, topCenter) {
    ...
    if (Ext.isObject(message)) {
      ...
      topCenter = message.topCenter;
      config = Wb.exclude(message, 'html,isError,topCenter');
    } else {
      html = message;
      config = null;
    }
    new Ext.ux.window.Notification(Wb.apply({
      ...
      position: topCenter ? 'tc' : 'br',
      iconCls: isError ? 'error_icon' : 'info_icon',
      html: html,
      minWidth: 150,
      maxWidth: 500,
      draggable: true,
  ...
  emptyFn: function() {},
  /**
   * 判断值是否为空。如果值为null, undefined, 空串， 空对象或数组长度为0，则为空。
   *
   * @param {Object} value 需要判断的值，可以为字符串，对象或数组等。
  ...
  init: function(sysData) {
    ...
    Wb.lang = sysData.lang;
    //统一监听beforeunload事件
    window.onbeforeunload = Wb.onBeforeUnload = function() {
      if (Wb.unloadEvents) {
        var result, value, container, count = 0;
        Wb.each(Wb.unloadEvents, function(name, fn) {
          value = fn();
          if (Wb.isValue(value)) {
            if (count === 0) {
              container = Ext.getCmp(name);
              if (container && container.ownerCt instanceof Ext.tab.Panel)
   ...
   * @param {Boolean} [options.maskTimeout] 指定当显示mask时，延时的毫秒数，默认为根据系统变量设置。
  ...
  request: function(options) {
    ...
    if (showMask)
      Wb.mask(newOptions.mask, newOptions.message, newOptions.maskTimeout);
  ...
  mask: function(component, message, maskTimeout) {
    var data, unionMsg, body = Ext.getBody(),
      timeout = Wb.isValue(maskTimeout) ? maskTimeout : Wb.maskTimeout;
  ...
  /**
   * 获取除使用getTextValue方法获得的显示值之外的所有值。详见Wb.getValue方法。
   */
  getStdValue: function(components, itemIds, getFileName) {
    return Wb.getValue(components, itemIds, getFileName, true);
  },
  /**
   * 获取指定控件及其所包含的所有控件的值和显示文本组成的对象，对象中每个值的名称为指定控件的itemId，显示文本的名称为指定控件的itemId加前缀“%”。
   * 只有具有getValue方法的控件的值才被提取，如果控件还具有getTextValue方法，则其显示文本也将被提取。
   * 如text控件将获得值，combo控件将获得值和显示文本，panel控件不获得值（添加getValue方法除外）。
   ...
   * @param {Boolean} [excludeTextValue] 是否排除使用getTextValue方法获得的显示值。
   * @return {Object} 获取的控件值组成的对象。
   */
  getValue: function(components, itemIds, getFileName, excludeTextValue) {
    var result = {},
      incTextValue = !excludeTextValue,
    ...
    function query(item) {
      ...
      if (itemId && item.getValue && (!itemIds || Wb.indexOf(itemIds, itemId) != -1) &&
        ...
        if (incTextValue && item.getTextValue) { //获得显示值
  ...
  /**
   * 把当前的焦点转到下一个编辑框。
   * @return {TextField} 编辑框控件。如果未找到返回null。
   */
  focusNext: function() {
    var cmp, dom = Ext.Element.getActiveElement();
    if (dom && dom.id) {
      if (Ext.String.endsWith(dom.id, '-inputEl')) {
        cmp = Ext.getCmp(dom.id.slice(0, -8));
        if (cmp) {
          if (cmp instanceof Ext.form.field.TextArea && !cmp.enterIsSpecial) {
            return null;
          }
          do {
            cmp = cmp.nextNode('textfield');
          } while (cmp && (Wb.isHidden(cmp) || cmp.disabled));
          if (cmp && !Wb.isHidden(cmp) && !cmp.disabled) {
  ...
  /**
   * 判断指定控件是否是隐藏的。如果控件所在的容器或更上级的容器隐藏则返回隐藏。
   * @param {Component} comp 需要判断的控件。
   * @return {Boolean} true隐藏，false不隐藏。
   */
  isHidden: function(comp) {
    ...
    do {
      if (ct.isHidden && ct.isHidden())
        return true;
      ct = isTouch ? ct.parent : ct.ownerCt;
    } while (ct);
    return false;
  },
  /**
   * 替换指定字符串首个source字符串为dest字符串。该方法同string.replace的区别为不使用正则表达式。
   * @param {String} string 需要替换的字符串。
   * @param {String} source 源字符串。
   * @param {String} dest 目标字符串。
   * @return {String} 替换后的字符串。
   */
  replace: function(string, source, dest) {
    if (!string || !source)
      return string;
    return string.replace(new RegExp(Wb.quoteRegexp(source)), dest);
  },
  /**
   * 全部替换指定字符串中所有source字符串为dest字符串。该方法同string.replace的区别为不使用正则表达式。
   * @param {String} string 需要替换的字符串。
   * @param {String} source 源字符串。
   * @param {String} dest 目标字符串。
   * @return {String} 替换后的字符串。
   */
  replaceAll: function(string, source, dest) {
    if (!string || !source)
      return string;
    return string.replace(new RegExp(Wb.quoteRegexp(source), 'g'), dest);
  },
  /**
  ...
  /**
   * 重置文件控件，并设置该文件控件的内容需要删除的标记。删除标记为[$fileItemId=1]。
   * @param {Ext.field.File file} fileField 需要删除子项的对象。
   */
  removeFile: function(fileField) {
    fileField.reset();
    fileField.removeFile = true;
  },
  /**
  ...
  /**
   * 获得数组/记录数据列表信息的标题。通常用于对该列表进行操作的提示。
   * @param {Array} items 数组或记录数组。
   * @param {String} fieldName 获取该字段名称的值作为标题。如果items为值数组，该值表示字段值。
   * @return {String} 记录列表信息标题。
   */
  getTitle: function(items, fieldName) {
    var title, count = items.length;
    if (count === 0)
      return null;
    else {
      if (items[0] instanceof Ext.data.Model)
        title = items[0].data[fieldName];
      else {
        if (Wb.isValue(fieldName))
          title = fieldName;
        else
          title = items[0];
      }
  ...
  exportData: function(component, type, isAll) {
    ...
    function getHeaders(columns, headers) {
      Wb.each(columns, function(col) {
        if (col.xtype == 'rownumberer' || col instanceof Ext.grid.column.RowNumberer) {
          rowNumberWidth = (col.getWidth ? col.getWidth() : col.width) || col.width;
        ...
        if (Ext.isTouch) {
          ...
          if (col.items && col.items.length) {
            ...
            data.width = (col.getWidth ? col.getWidth() : col.width) || col.width; //列隐藏时getWidth()=0
    ...
    var params, url, urlParams, charIndex, rowNumberWidth, rowNumberTitle,
      fieldTypes, grid, store, mergeInfo, fields;
    if (component instanceof Ext.toolbar.Paging) {
      if (component.ownerCt instanceof Ext.grid.Panel)
        grid = component.ownerCt;
      else
        grid = component.ownerCt.items.items[0];
    } else
    ...
    params = Wb.applyIf({
      __metaParams: {
        ...
        reportInfo: {
          ...
          topHtml: grid.down ? getHtmlData(grid.down('#topHtml')) : '',
          bottomHtml: grid.down ? getHtmlData(grid.down('#bottomHtml')) : ''
        },
  ...
  /**
   * 获得指定容器下指定组名中被按下的按钮。如果没有任何按钮被按下返回null。
   * @param {Container} owner 容器对象。
   * @param {String} groupName 按钮组名称。
   * @return {Button} 被按下的按钮或null。
   */
  getPressed: function(owner, groupName) {
    var btn = null;
    owner.items.each(function(item) {
      if (item.toggleGroup == groupName && item.pressed) {
        btn = item;
        return false;
      }
    });
   ...
   * @param {Number} [editField] 激活编辑单元格所在的列对象或索引号。如果指定该值在添加记录后自动激活该列单元格。-1表示激活首个可以编辑的列。默认不自动激活。
    ...
    if (editing && editField !== undefined) {
      if (editField == -1) {
        ...
        Wb.each(cols, function(col) {
          ...
          if (editor && !editor.readOnly && !col.hidden) {
            editField = pos;
            return false;
          }
          pos++;
  ...
  /**
   * 在表格末尾添加新的记录，并对新记录首个可编辑列开启编辑状态。详细说明见Wb.add。
   */
   ...
   * @param {Object} configs 配置参数。
   * @param {Boolean} configs.autoUpdate 是否自动根据返回值同步表格中的数据。
   * 返回值的格式为{create:createRecs,update:updateRecs}。默认为false。
  ...
  sync: function(configs) {
    var editing, syncFn, oldSuccssFn, store = configs.store || configs.grid.store;
    if (store.bindTable) {
    ...
    if (configs.grid)
      delete configs.grid;
    if (store.bindTable && configs.autoUpdate) {
      syncFn = function(data) {
        data = Wb.decode(data);
        if (data.create)
          Wb.syncCreate(store.bindTable, data.create, !data.update);
        if (data.update)
          Wb.syncUpdate(store.bindTable, data.update, true);
      };
      if (configs.success) {
        oldSuccssFn = configs.success;
        configs.success = function(resp) {
          syncFn(resp.responseText);
          Ext.callback(oldSuccssFn, this, arguments);
        };
      } else {
        configs.success = syncFn;
      }
    }
    Wb.request(configs);
  },
   ...
   * @param {Boolean} configs.maskTimeout 指定当显示mask时，延时的毫秒数，默认为根据系统变量设置。
  ...
  kvRenderer: window.Ext && Ext.isTouch ? function(value, r) {
    ...
    return Wb.kv(value, cols[colIndex].initialConfig.keyItems);
  ...
  /**
   * 生成指定编码的Glyph图标脚本。该脚本通常可用于表格、树、下拉框、HTML等。
   * @param {String} glyph Glyph编码。
   * @param {String} [title] 图标提示文本。
   * @param {String} [color] 图标颜色，默认为不设置颜色。
   * @return {String} HTML脚本。
   */
  getGlyph: function(glyph, title, color) {
    if (Wb.isEmpty(title))
      title = '';
    else
      title = ' title=' + Wb.encode(title);
    ...
    return '<span class="wb_glyph wb_glyph_icon"' + title + color + '>&#x' + glyph + ';</span>';
  },
  /**
  ...
  setChecked: function(node, checked, tree, ignoreBox) {
    ...
    if (isNull) {
      ...
      el.setStyle('color', '');
      if (box) {
        box.removeCls('x-tree-checkbox-checked');
        box.addCls('x-tree-checkbox-halfchecked');
      }
    } else if (checked) {
      el.setStyle('text-decoration', '');
      el.setStyle('color', 'blue');
      if (box) {
        box.removeCls('x-tree-checkbox-halfchecked');
        box.addCls('x-tree-checkbox-checked');
      }
    } else {
  ...
  /**
   * 获取iframe所属的Ext窗口对象。
   * @param {window} win 窗口对象，可为parent或top等。
   * @return {Ext.window.Window} Ext窗口对象。
   */
  getOwnerWin: function(win) {
    if (!win)
      win = parent;
    var el = win.Ext.fly(window.frameElement).up('div.x-window');
    return win.Ext.getCmp(el.id);
  },
  /**
   * 对参数列表中的所有控件设置禁用状态。
   ...
   * @param {Boolean} [PortalConfigs.autoTab] 是否自动创建标签的标题和图标，此项仅当打开xwl模块时有效。
   * @param {Boolean} [PortalConfigs.topWin] 是否在顶层窗口中打开，默认为false。   
  ...
  open: function(PortalConfigs) {
    if (PortalConfigs.topWin && !Ext.isTouch && window.top != window && window.top.Wb && window.top.sys &&
      (window.top.sys.home || window.top.sys.ide)) {
      ...
      Wb.request({
        ...
        success: function(resp) {
          if (resp.responseText) {
            PortalConfigs = Wb.apply({}, PortalConfigs, Wb.decode(resp.responseText));
              ...
              if (PortalResp.responseText) {
                PortalVars.doOpen = function() {
                  ...
                  if (PortalResp.responseText.substring(0, 10) == '(function(' && PortalResp.responseText.slice(-3) == '();') {
                    ...
                    PortalVars.appScope = eval(PortalResp.responseText.slice(0, -2) +
                      ...
                      PortalVars.loc.protocol + "//" + PortalVars.loc.host + PortalVars.loc.pathname.substring(0, PortalVars.loc.pathname.lastIndexOf('/')) + '/' + PortalVars.path);
                    //sourceURL用于在控制台标识文件
                ...
                if (Ext.String.startsWith(PortalResp.responseText, '$$@blink')) {
                  ...
                  if (PortalVars.scriptObject.css) {
                    Wb.each(PortalVars.scriptObject.css, function(item) {
                  ...
                  if (PortalVars.scriptObject.js) {
                    Wb.each(PortalVars.scriptObject.js, function(item) {
                      PortalVars.linksArray.push({
                        url: item,
                        type: 'js'
                      });
   ...
   * 该方法同Wb.open方法的区别为运行时不创建默认容器。详细说明见{@link #open}
   * Example:
   *
   ...
   *     }});
   *
   * @param {Boolean} [single] 是否使用单例模式运行。单例模式下对同一url的方法调用，第二次及以后将直接回调sucess方法
   * 并传递上次相同url生成的appScope参数,responseText参数为undefined。
   */
  run: function(configs) {
    ...
    if (configs.single) {
      var scope = Wb.singleScopes[configs.url];
      if (scope) {
        Ext.callback(configs.success, scope, [scope]);
        return;
      } else {
        cfg.success = function(scope) {
          Wb.singleScopes[configs.url] = scope;
          Ext.callback(configs.success, scope, [scope]);
        };
      }
    }
    Ext.applyIf(cfg, configs);
    Wb.open(cfg);
  },
  /**
   ...
   * @param {Boolean} [topWin] 是否关闭顶层窗口中的相关页面，默认为false。
   */
  close: function(value, topWin) {
    if (topWin && !Ext.isTouch && window.top != window && window.top.Wb && window.top.sys &&
      (window.top.sys.home || window.top.sys.ide)) {
  ...
  /**
   * 关闭标签页中的指定页面。
   * @param {TabPanel} tabpanel 标签页对象。
   * @param {Boolean} [type] 关闭类型。true关闭全部，false关闭当前页外的其他页。默认为关闭当前页。
   */
  closeTab: function(tabpanel, type) {
    var card = tabpanel.getActiveTab();
    if (Wb.isValue(type)) {
  ...
  print: function(object, title) {
    ...
    if (Wb.isEmpty(object))
      ...
      el = object.el.down('#' + object.id + '-innerCt');
      if (!el) {
        if (object.body)
          el = object.body;
        else
  ...
  /**
   * 读取文件控件中的文件数据并显示到图片控件中。
   * @param {Ext.form.field.File} file 文件控件。
   * @param {Ext.Img/img} image Image控件或Image dom。
   */
  readImage: function(file, image) {
    try {
      var input = file.fileInputEl.dom;
      if (input.files && input.files[0]) {
        if (!Wb.fileReader)
          Wb.fileReader = new FileReader();
        Wb.fileReader.onload = function(e) {
          if (image instanceof Ext.Img)
            image.setSrc(e.target.result);
          else image.src = e.taget.result;
        };
        Wb.fileReader.readAsDataURL(input.files[0]);
      }
    } catch (e) {}
  },
  /**
   * 销毁指定命名空间下的所有资源。执行该方法后所有无上级控件的控件均会被销毁，但存在上级控件的控件不会被销毁（通常由上级控件销毁时销毁）。
   * @param {Object} scope 命名空间。
   * @param {Component} [exceptComp] 例外对象。如果指定该对象调用该方法后不会销毁该对象。
   */
  destroy: function(scope, exceptComp) {
    if (!scope) return;
    if (Ext.isTouch) {
      Wb.each(scope, function(k, v) {
        if (v && v != exceptComp && (!Ext.isFunction(v.up) || !v.up()) && !(v instanceof Ext.data.Model) && Ext.isFunction(v.destroy)) {
          try {
            v.destroy();
          } catch (e) {}
        } else if (Ext.isObject(v) && v.isXNS && k != 'contextOwner') {
          Wb.destroy(v, exceptComp);
        }
      });
    } else {
      Wb.each(scope, function(k, v) {
        if (v && v != exceptComp) {
          try {
            if (v.isStore)
              v.destroyStore();
            else if (!v.ownerCt && !(v instanceof Ext.data.Model) && Ext.isFunction(v.destroy))
              v.destroy();
            else if (Ext.isObject(v) && v.isXNS && k != 'contextOwner') {
              Wb.destroy(v, exceptComp);
            }
  ...
  /**
   * 把两个数组按 displayItem1 (valueItem1), displayItem2 (valueItem2)的形式合并成字符串。
   * 注：两个数组的长度必须一致。
   * @param {Array} displayArray 显示数组。
   * @param {Array} [valueArray] 值数组，显示在括号内。如果缺省将不显示该值
   * @return {String} 合并后的字符串。
   */
  joinArray: function(displayArray, valueArray) {
    var val = [],
      i, j = displayArray.length;
    if (j === 0) return '';
    if (!valueArray)
      return displayArray.join(', ');
    for (i = 0; i < j; i++)
      val.push(displayArray[i] + ' (' + valueArray[i] + ')');
    return val.join(', ');
  },
  /**
